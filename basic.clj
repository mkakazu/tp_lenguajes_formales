    ...
    (if (or (nil? operando1) (nil? operando2))
        ...
        (case operador
          ...
          * (if (and (number? operando1) (number? operando2))
          ...
          OR (let [op1 (+ 0 operando1), op2 (+ 0 operando2)] (do (if (or (not= op1 0) (not= op2 0)) 1 0)))
          MID$ (if (< operando2 1)
...
(defn palabra-reservada? [x]
  (case x
    (PRINT LOAD SAVE REM NEW RUN GOTO IF INPUT ON GOSUB RETURN FOR NEXT INT ATN OR AND LEN MID$ STEP SIN CHR$ ASC MID3$ STR$) true
    false))
...
(defn dar-error [cod prog-ptrs]
  (print (str "\n"
              (buscar-mensaje cod)
              (if (number? (get prog-ptrs 0)) (str " IN " (get prog-ptrs 0))))))
